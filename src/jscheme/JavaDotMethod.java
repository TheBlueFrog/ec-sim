...
public class JavaDotMethod extends Procedure {
	...
	protected boolean isMatch(Class<?>[] types, Object[] objs)
	{
		...
		for (int i = 0; i < objs.length; i++)
		{
			...
			else if (types[i].isAssignableFrom(String.class) && objs[i].getClass() == char[].class)
			{
				continue;
			}
			else if (types[i].isAssignableFrom(int.class) && objs[i].getClass() == Double.class)
			{
				continue;
			}
			else if (types[i].isAssignableFrom(long.class) && objs[i].getClass() == Double.class)
			{
				continue;
			}
			else if (types[i].isAssignableFrom(float.class) && objs[i].getClass() == Double.class)
			{
				continue;
			}
			else if (types[i].isAssignableFrom(double.class) && objs[i].getClass() == Double.class)
			{
				continue;
			}
			else if (types[i] == int.class && objs[i].getClass() == Integer.class)
			{
				continue;
			}
			else if (types[i] == long.class && objs[i].getClass() == Integer.class)
			{
				continue;
			}
			// throw new
			// RuntimeException("Bailing on: "+types[i]+"<->"+objs[i].getClass());
			return false;
		}
		return true;
	}
	...
	protected Object[] coerce(Class<?>[] types, Object[] objs)
	{
		...
		for (int i = 0; i < res.length; i++)
		{
			...
			{
				res[i] = Integer.valueOf(((Double) objs[i]).intValue());// new Integer(((Double) objs[i]).intValue());
			}
			else if (types[i].isAssignableFrom(long.class) && objs[i].getClass() == Double.class)
			{
				res[i] = Long.valueOf(((Double) objs[i]).longValue());//new Long(((Double) objs[i]).longValue());
			}
			else if (types[i].isAssignableFrom(float.class) && objs[i].getClass() == Double.class)
			{
				res[i] = Float.valueOf(((Double) objs[i]).floatValue());//new Float(((Float) objs[i]).floatValue());
			}
			else if (types[i].isAssignableFrom(double.class) && objs[i].getClass() == Double.class)
			{
				res[i] = (Double) objs[i];
			...
			{
				res[i] = ((Integer) objs[i]).longValue();
			}
			else
			{
	...
	protected Object doConstructor(Scheme interpreter, Object args)
		...
		error("Couldn't find constructor matching " + className);
		return null;
	}
	...
	protected Object doMethod(Scheme interpreter, String name, Object target, Class<?> clazz, Object args)
			throws InvocationTargetException, IllegalAccessException
	{
		Object[] as = getArgs(args);
		...
		{
			if (ms[i].getName().equals(name) && isMatch(ms[i].getParameterTypes(), as))
			{
				return ms[i].invoke(target, coerce(ms[i].getParameterTypes(), as));
			}
		}

		error("Couldn't find usable method " + name);
		return null;
	}
	...
	protected Object doField(Scheme interpreter, String name, Class<?> clazz,
		...
		error("No such field: " + method + " " + name);
		return null;
	}
